...
def _render_settings(
    ...
    selected_moulding_tool = (
        next((tool for tool in moulding_tools if tool.id == tool_id), None) if tool_id else None
    )
    selected_moulding_machine = (
        next((machine for machine in moulding_machines if machine.id == machine_id), None)
        if machine_id
        else None
    )
    assembly_lines = context["assembly_lines"]
    selected_assembly_line = (
        next((line for line in assembly_lines if line.id == assembly_line_id), None)
        if assembly_line_id
        else None
    )
    metalization_masks = context["metalization_masks"]
    selected_metalization_mask = (
        next((mask for mask in metalization_masks if mask.id == mask_id), None) if mask_id else None
    )
    metalization_chambers = context["metalization_chambers"]
    selected_metalization_chamber = (
        next((chamber for chamber in metalization_chambers if chamber.id == chamber_id), None)
        if chamber_id
        else None
    )
    materials = context["materials"]
    selected_material = (
        next((material for material in materials if material.id == material_id), None)
        if material_id
        else None
    )
    selected_moulding_tool_hc_map = (
        settings_service.get_tool_hc_map(db, selected_moulding_tool.id)
        if selected_moulding_tool
        else {}
    )
    selected_metalization_mask_hc_map = (
        settings_service.get_mask_hc_map(db, selected_metalization_mask.id)
        if selected_metalization_mask
        else {}
    )
    selected_moulding_tool_materials = (
        settings_service.list_materials_for_tool(db, selected_moulding_tool.id)
        if selected_moulding_tool
        else []
    )
    selected_moulding_tool_materials_out = (
        settings_service.list_materials_out_for_tool(db, selected_moulding_tool.id)
        if selected_moulding_tool
        else []
    )
    selected_metalization_mask_materials = (
        settings_service.list_materials_for_mask(db, selected_metalization_mask.id)
        if selected_metalization_mask
        else []
    )
    selected_metalization_mask_materials_out = (
        settings_service.list_materials_out_for_mask(db, selected_metalization_mask.id)
        if selected_metalization_mask
        else []
    )
    selected_assembly_line_hc_map = (
        settings_service.get_assembly_line_hc_map(db, selected_assembly_line.id)
        if selected_assembly_line
        else {}
    )
    selected_assembly_line_materials_in = (
        settings_service.list_materials_in_for_assembly_line(db, selected_assembly_line.id)
        if selected_assembly_line
        else []
    )
    selected_assembly_line_materials_out = (
        settings_service.list_materials_out_for_assembly_line(db, selected_assembly_line.id)
        if selected_assembly_line
        else []
    )
...
def settings_champions_page(
    ...
    return _render_settings(
        ...
        project_id=None,
        message=message
        or ("Champion added" if (created or "").strip().lower() == "champion" else None)
        or ("Champion deleted" if (deleted or "").strip().lower() == "champion" else None),
        error=error,
    )
...
def settings_projects_page(
    ...
    return _render_settings(
        ...
        message=message
        or ("Project added" if (created or "").strip().lower() == "project" else None),
        error=error,
    )
...
def settings_moulding_tools_page(
    ...
    return _render_settings(
        ...
        tool_id=tool_id,
        message=message
        or ("Moulding tool added" if (created or "").strip().lower() == "moulding_tool" else None),
        error=error,
    )
...
def settings_moulding_machines_page(
    ...
    return _render_settings(
        ...
        message=message
        or (
            "Moulding machine added"
            if (created or "").strip().lower() == "moulding_machine"
            else None
        ),
        error=error,
    )
...
@router.get(
    "/settings/moulding-machines/{machine_id}/tools",
...
def settings_assembly_lines_page(
    ...
    return _render_settings(
        ...
        project_id=None,
        message=message
        or ("Assembly line added" if (created or "").strip().lower() == "assembly_line" else None),
        error=error,
...
def _render_reference_settings_page(
    ...
    selected_reference = (
        next((ref for ref in references if ref.id == reference_id), None) if reference_id else None
    )
    selected_reference_hc_map = (
        settings_service.get_assembly_line_reference_hc_map(db, selected_reference.id)
        if selected_reference
        else {}
    )
    selected_reference_materials_in = (
        settings_service.list_materials_in_for_reference(db, selected_reference.id)
        if selected_reference
        else []
    )
    selected_reference_materials_out = (
        settings_service.list_materials_out_for_reference(db, selected_reference.id)
        if selected_reference
        else []
    )
    return templates.TemplateResponse(
...
@router.get(
    "/settings/assembly-lines/{line_id}/references",
    response_class=HTMLResponse,
    response_model=None,
)
def settings_assembly_line_references_page(
    ...
    return _render_reference_settings_page(
        ...
        message=message
        or ("Reference added" if (created or "").strip().lower() == "reference" else None),
        error=error,
    )
...
def settings_metalization_masks_page(
    ...
    return _render_settings(
        ...
        message=message
        or (
            "Metalization mask added"
            if (created or "").strip().lower() == "metalization_mask"
            else None
        ),
        error=error,
        mask_id=mask_id,
...
def settings_metalization_chambers_page(
    ...
    return _render_settings(
        ...
        message=message
        or (
            "Metalization chamber added"
            if (created or "").strip().lower() == "metalization_chamber"
            else None
        ),
        error=error,
...
@router.get(
    "/settings/metalization-chambers/{chamber_id}/masks",
    response_class=HTMLResponse,
    response_model=None,
)
...
def settings_materials_page(
    ...
    return _render_settings(
        ...
        message=message
        or ("Material added" if (created or "").strip().lower() == "material" else None),
        error=error,
    )
...
def update_user_role(
    ...
    try:
        ...
        return _render_settings(
            "settings_users.html", request, db, champion_id=None, project_id=None, error=str(exc)
        )
...
def add_champion(
    ...
    return RedirectResponse(
        url=f"/ui/settings/champions?created=champion&champion_id={champion.id}", status_code=303
    )
...
def update_champion(
    ...
    return RedirectResponse(
        url=f"/ui/settings/champions?message=Champion+updated&champion_id={champion.id}",
        status_code=303,
    )


@router.post("/settings/champions/{champion_id}/delete", response_model=None)
def delete_champion(
    champion_id: int,
    request: Request,
    db: Session = Depends(get_db),
...
def add_project(
    ...
    return RedirectResponse(
        url=f"/ui/settings/projects?created=project&project_id={project.id}", status_code=303
    )
...
def update_project(
    ...
    return RedirectResponse(
        url=f"/ui/settings/projects?message=Project+updated&project_id={project.id}",
        status_code=303,
    )
...
def add_project_tool(
    ...
    try:
        ...
        settings_service.add_project_moulding_tool(
            db, project_id=project_id, tool_id=parsed_tool_id
        )
    except ValueError as exc:
        return RedirectResponse(
...
def remove_project_tool(
    ...
    try:
        ...
        settings_service.remove_project_moulding_tool(
            db, project_id=project_id, tool_id=parsed_tool_id
        )
    except ValueError as exc:
        return RedirectResponse(
...
def add_project_line(
    ...
    try:
        if line_number is not None:
            ...
            raise ValueError("Assembly line is required.")
        settings_service.add_project_assembly_line(
            db, project_id=project_id, line_id=parsed_line_id
        )
    except ValueError as exc:
        return RedirectResponse(
            url=f"/ui/settings/projects?project_id={project_id}&error={quote_plus(str(exc))}",
...
def remove_project_line(
    ...
    try:
        parsed_line_id = _resolve_line_id(db, line_id)
        settings_service.remove_project_assembly_line(
            db, project_id=project_id, line_id=parsed_line_id
        )
    except ValueError as exc:
        return RedirectResponse(
...
def add_project_metalization_mask(
    ...
    try:
        ...
        settings_service.add_project_metalization_mask(
            db, project_id=project_id, mask_id=parsed_mask_id
        )
    except ValueError as exc:
        return RedirectResponse(
            url=f"/ui/settings/projects?project_id={project_id}&error={quote_plus(str(exc))}",
...
def remove_project_metalization_mask(
    ...
    try:
        ...
        settings_service.remove_project_metalization_mask(
            db, project_id=project_id, mask_id=parsed_mask_id
        )
    except ValueError as exc:
        return RedirectResponse(
...
@router.get(
    "/settings/projects/{project_id}/assignments", response_class=HTMLResponse, response_model=None
)
def project_assignments_page(
    ...
    if project is None:
        return RedirectResponse(
            url="/ui/settings/projects?error=Project+not+found", status_code=303
        )
    return templates.TemplateResponse(
...
def add_moulding_tool(
    ...
    try:
        hc_map = _build_hc_map(
            hc_operator, hc_logistic, hc_teamleader, hc_inspector, hc_specialist, hc_technican
        )
    ...
    return RedirectResponse(
        url=f"/ui/settings/moulding-tools?created=moulding_tool&tool_id={tool.id}", status_code=303
    )
...
def update_moulding_tool(
    ...
    return RedirectResponse(
        url=f"/ui/settings/moulding-tools?message=Moulding+tool+updated&tool_id={tool.id}",
        status_code=303,
    )
...
def delete_moulding_tool(tool_id: int, request: Request, db: Session = Depends(get_db)):
    ...
    return RedirectResponse(
        url="/ui/settings/moulding-tools?message=Moulding+tool+deleted", status_code=303
    )
...
def add_moulding_tool_material(
    ...
    try:
        ...
        return RedirectResponse(
            url=f"/ui/settings/moulding-tools?tool_id={tool_id}&error={quote_plus(str(exc))}",
            status_code=303,
        )
    return RedirectResponse(
        url=f"/ui/settings/moulding-tools?tool_id={tool_id}&message=Material+added", status_code=303
    )
...
def update_moulding_tool_material_qty(
    ...
    return RedirectResponse(
        url=f"/ui/settings/moulding-tools?tool_id={tool_id}&message=Material+qty+updated",
        status_code=303,
    )


@router.post(
    "/settings/moulding-tools/{tool_id}/materials/{material_id}/remove", response_model=None
)
def remove_moulding_tool_material(
    ...
    return RedirectResponse(
        url=f"/ui/settings/moulding-tools?tool_id={tool_id}&message=Material+removed",
        status_code=303,
    )
...
def add_moulding_tool_material_out(
    ...
    return RedirectResponse(
        url=f"/ui/settings/moulding-tools?tool_id={tool_id}&message=Outcome+material+added",
        status_code=303,
    )


@router.post(
    "/settings/moulding-tools/{tool_id}/materials-out/{material_id}/qty", response_model=None
)
def update_moulding_tool_material_out_qty(
    tool_id: int,
    material_id: int,
    request: Request,
    qty_per_piece: str = Form(...),
    ...
    return RedirectResponse(
        url=f"/ui/settings/moulding-tools?tool_id={tool_id}&message=Outcome+material+qty+updated",
        status_code=303,
    )


@router.post(
    "/settings/moulding-tools/{tool_id}/materials-out/{material_id}/remove", response_model=None
)
def remove_moulding_tool_material_out(
    tool_id: int, material_id: int, request: Request, db: Session = Depends(get_db)
):
    enforce_admin(_current_user(request))
    try:
    ...
    return RedirectResponse(
        url=f"/ui/settings/moulding-tools?tool_id={tool_id}&message=Outcome+material+removed",
        status_code=303,
    )
...
def update_moulding_machine(
    ...
    try:
        parsed_tonnage = _parse_optional_int(tonnage, "Tonnage")
        parsed_tool_ids = (
            _parse_int_list(tool_ids, "Tools assigned") if tool_ids is not None else None
        )
        if parsed_tool_ids is None:
            existing_machine = next(
                (m for m in settings_service.list_moulding_machines(db) if m.id == machine_id), None
            )
            parsed_tool_ids = (
                [tool.id for tool in existing_machine.tools] if existing_machine else []
            )
...
def add_moulding_machine_tool(
    ...
    try:
        parsed_tool_id = (
            _parse_optional_int(tool_id, "Moulding tool") if tool_id is not None else None
        )
    ...
    return RedirectResponse(
        url=f"/ui/settings/moulding-machines?machine_id={machine_id}", status_code=303
    )
...
def delete_moulding_machine(machine_id: int, request: Request, db: Session = Depends(get_db)):
    ...
    return RedirectResponse(
        url="/ui/settings/moulding-machines?message=Moulding+machine+deleted", status_code=303
    )
...
def add_assembly_line(
    ...
    try:
        ...
        assembly_line = settings_service.create_assembly_line(
            db,
            AssemblyLineCreate(
                line_number=line_number, ct_seconds=float(ct_seconds), hc=0, hc_map=hc_map
            ),
        )
    except (ValidationError, ValueError, TypeError) as exc:
        return _render_settings(
...
def update_assembly_line(
    ...
    try:
        ...
        assembly_line = settings_service.update_assembly_line(
            ...
            assembly_line_id,
            AssemblyLineUpdate(
                line_number=line_number, ct_seconds=float(ct_seconds), hc_map=hc_map
            ),
        )
    except (ValidationError, ValueError, TypeError) as exc:
        return _render_settings(
...
def delete_assembly_line(assembly_line_id: int, request: Request, db: Session = Depends(get_db)):
    ...
    return RedirectResponse(
        url="/ui/settings/assembly-lines?message=Assembly+line+deleted", status_code=303
    )
...
def update_assembly_line_hc(
    ...
    enforce_admin(_current_user(request))
    try:
        settings_service.set_assembly_line_hc(
            db,
            assembly_line_id,
            _build_hc_map(
                hc_operator, hc_logistic, hc_teamleader, hc_inspector, hc_specialist, hc_technican
        ...
        return RedirectResponse(
            url=f"/ui/settings/assembly-lines?assembly_line_id={assembly_line_id}&error={quote_plus(str(exc))}",
            status_code=303,
        )
    return RedirectResponse(
        url=f"/ui/settings/assembly-lines?assembly_line_id={assembly_line_id}&message=HC+updated",
        status_code=303,
    )


@router.post("/settings/assembly-lines/{assembly_line_id}/materials-in/add", response_model=None)
def add_assembly_line_material_in(
    assembly_line_id: int,
    request: Request,
    part_number: str | None = Form(default=None),
    material_id: str | None = Form(default=None),
    ...
    return RedirectResponse(
        url=f"/ui/settings/assembly-lines?assembly_line_id={assembly_line_id}&message=Material+added",
        status_code=303,
    )


@router.post(
    "/settings/assembly-lines/{assembly_line_id}/materials-in/{material_id}/qty",
    response_model=None,
)
def update_assembly_line_material_in_qty(
    ...
    try:
        ...
        if parsed_qty is None:
            raise ValueError("Qty / piece is required.")
        settings_service.update_assembly_line_material_in_qty(
            db, assembly_line_id, material_id, parsed_qty
        )
    except ValueError as exc:
        return RedirectResponse(
            url=f"/ui/settings/assembly-lines?assembly_line_id={assembly_line_id}&error={quote_plus(str(exc))}",
    ...
    return RedirectResponse(
        url=f"/ui/settings/assembly-lines?assembly_line_id={assembly_line_id}&message=Material+qty+updated",
        status_code=303,
    )


@router.post(
    "/settings/assembly-lines/{assembly_line_id}/materials-in/{material_id}/remove",
    response_model=None,
)
def remove_assembly_line_material_in(
    assembly_line_id: int, material_id: int, request: Request, db: Session = Depends(get_db)
):
    enforce_admin(_current_user(request))
    try:
    ...
    return RedirectResponse(
        url=f"/ui/settings/assembly-lines?assembly_line_id={assembly_line_id}&message=Material+removed",
        status_code=303,
    )


@router.post("/settings/assembly-lines/{assembly_line_id}/materials-out/add", response_model=None)
def add_assembly_line_material_out(
    assembly_line_id: int,
    request: Request,
    part_number: str | None = Form(default=None),
    material_id: str | None = Form(default=None),
    ...
    return RedirectResponse(
        url=f"/ui/settings/assembly-lines?assembly_line_id={assembly_line_id}&message=Outcome+material+added",
        status_code=303,
    )


@router.post(
    "/settings/assembly-lines/{assembly_line_id}/materials-out/{material_id}/qty",
    response_model=None,
)
def update_assembly_line_material_out_qty(
    ...
    try:
        ...
        if parsed_qty is None:
            raise ValueError("Qty / piece is required.")
        settings_service.update_assembly_line_material_out_qty(
            db, assembly_line_id, material_id, parsed_qty
        )
    except ValueError as exc:
        return RedirectResponse(
    ...
    return RedirectResponse(
        url=f"/ui/settings/assembly-lines?assembly_line_id={assembly_line_id}&message=Outcome+material+qty+updated",
        status_code=303,
    )


@router.post(
    "/settings/assembly-lines/{assembly_line_id}/materials-out/{material_id}/remove",
    response_model=None,
)
def remove_assembly_line_material_out(
    assembly_line_id: int, material_id: int, request: Request, db: Session = Depends(get_db)
):
    enforce_admin(_current_user(request))
    try:
    ...
    return RedirectResponse(
        url=f"/ui/settings/assembly-lines?assembly_line_id={assembly_line_id}&message=Outcome+material+removed",
        status_code=303,
    )
...
def add_metalization_mask(
    ...
    try:
        ...
        return _render_settings(
            ...
            form={
                "mask_pn": mask_pn,
                "mask_description": description or "",
                "mask_ct_seconds": ct_seconds,
                **{f"hc_{k.lower()}": v for k, v in hc_map.items()},
            },
            open_modal="metalization-mask",
        )
    return RedirectResponse(
        url=f"/ui/settings/metalization-masks?created=metalization_mask&mask_id={mask.id}",
        status_code=303,
    )
...
def update_metalization_mask(
    ...
    return RedirectResponse(
        url=f"/ui/settings/metalization-masks?message=Metalization+mask+updated&mask_id={mask.id}",
        status_code=303,
    )
...
def delete_metalization_mask(mask_id: int, request: Request, db: Session = Depends(get_db)):
    ...
    return RedirectResponse(
        url="/ui/settings/metalization-masks?message=Metalization+mask+deleted", status_code=303
    )
...
def add_metalization_mask_material(
    ...
    try:
        ...
        return RedirectResponse(
            url=f"/ui/settings/metalization-masks?mask_id={mask_id}&error={quote_plus(str(exc))}",
            status_code=303,
        )
    return RedirectResponse(
        url=f"/ui/settings/metalization-masks?mask_id={mask_id}&message=Material+added",
        status_code=303,
    )


@router.post(
    "/settings/metalization-masks/{mask_id}/materials/{material_id}/qty", response_model=None
)
def update_metalization_mask_material_qty(
    ...
    return RedirectResponse(
        url=f"/ui/settings/metalization-masks?mask_id={mask_id}&message=Material+qty+updated",
        status_code=303,
    )


@router.post(
    "/settings/metalization-masks/{mask_id}/materials/{material_id}/remove", response_model=None
)
def remove_metalization_mask_material(
    ...
    return RedirectResponse(
        url=f"/ui/settings/metalization-masks?mask_id={mask_id}&message=Material+removed",
        status_code=303,
    )


@router.post("/settings/metalization-masks/{mask_id}/materials-out/add", response_model=None)
def add_metalization_mask_material_out(
    mask_id: int,
    request: Request,
    part_number: str | None = Form(default=None),
    material_id: str | None = Form(default=None),
    ...
    return RedirectResponse(
        url=f"/ui/settings/metalization-masks?mask_id={mask_id}&message=Outcome+material+added",
        status_code=303,
    )


@router.post(
    "/settings/metalization-masks/{mask_id}/materials-out/{material_id}/qty", response_model=None
)
def update_metalization_mask_material_out_qty(
    mask_id: int,
    material_id: int,
    request: Request,
    qty_per_piece: str = Form(...),
    ...
    return RedirectResponse(
        url=f"/ui/settings/metalization-masks?mask_id={mask_id}&message=Outcome+material+qty+updated",
        status_code=303,
    )


@router.post(
    "/settings/metalization-masks/{mask_id}/materials-out/{material_id}/remove", response_model=None
)
def remove_metalization_mask_material_out(
    mask_id: int, material_id: int, request: Request, db: Session = Depends(get_db)
):
    enforce_admin(_current_user(request))
    try:
    ...
    return RedirectResponse(
        url=f"/ui/settings/metalization-masks?mask_id={mask_id}&message=Outcome+material+removed",
        status_code=303,
    )
...
def add_metalization_chamber(
    ...
    try:
        chamber = settings_service.create_metalization_chamber(
            db,
            MetalizationChamberCreate(
                chamber_number=chamber_number, mask_ids=_parse_int_list(mask_ids, "Masks assigned")
            ),
        )
    except (ValidationError, ValueError) as exc:
        return _render_settings(
    ...
    return RedirectResponse(
        url=f"/ui/settings/metalization-chambers?created=metalization_chamber&chamber_id={chamber.id}",
        status_code=303,
    )
...
def update_metalization_chamber(
    ...
    try:
        parsed_mask_ids = (
            _parse_int_list(mask_ids, "Masks assigned") if mask_ids is not None else None
        )
        if parsed_mask_ids is None:
            existing_chamber = next(
                (c for c in settings_service.list_metalization_chambers(db) if c.id == chamber_id),
                None,
            )
            parsed_mask_ids = (
                [mask.id for mask in existing_chamber.masks] if existing_chamber else []
            )
    ...
    return RedirectResponse(
        url=f"/ui/settings/metalization-chambers?message=Metalization+chamber+updated&chamber_id={chamber.id}",
        status_code=303,
    )
...
def add_metalization_chamber_mask(
    ...
    try:
        parsed_mask_id = (
            _parse_optional_int(mask_id, "Metalization mask") if mask_id is not None else None
        )
    ...
    return RedirectResponse(
        url=f"/ui/settings/metalization-chambers?chamber_id={chamber_id}", status_code=303
    )
...
def delete_metalization_chamber(chamber_id: int, request: Request, db: Session = Depends(get_db)):
    ...
    return RedirectResponse(
        url="/ui/settings/metalization-chambers?message=Metalization+chamber+deleted",
        status_code=303,
    )
...
def update_labour_cost(
    ...
    return RedirectResponse(
        url="/ui/settings/labour-cost?message=Labour+cost+updated", status_code=303
    )
...
def edit_material(
    ...
    return RedirectResponse(
        url=f"/ui/settings/materials?material_id={material_id}&message=Material+updated",
        status_code=303,
    )
...
def create_assembly_line_reference(
    ...
    return RedirectResponse(
        url=f"/ui/settings/assembly-lines/{line_id}/references?created=reference&reference_id={reference.id}",
        status_code=303,
    )
...
def update_assembly_line_reference(
    ...
    return RedirectResponse(
        url=f"/ui/settings/assembly-lines/{ref.line_id}/references?message=Reference+updated&reference_id={reference_id}",
        status_code=303,
    )


@router.post("/settings/assembly-lines/references/{reference_id}/delete", response_model=None)
def delete_assembly_line_reference(
    reference_id: int, request: Request, db: Session = Depends(get_db)
):
    enforce_admin(_current_user(request))
    ref = settings_service.get_assembly_line_reference(db, reference_id)
    if ref is None:
    ...
    return RedirectResponse(
        url=f"/ui/settings/assembly-lines/{line_id}/references?message=Reference+deleted",
        status_code=303,
    )


@router.post(
    "/settings/assembly-lines/references/{reference_id}/materials-in/add", response_model=None
)
def add_reference_material_in(
    reference_id: int,
    request: Request,
    part_number: str = Form(...),
    qty_per_piece: str = Form(...),
    ...
    try:
        ...
        settings_service.add_material_in_to_reference(
            db, reference_id, part_number=part_number, qty_per_piece=parsed_qty
        )
    except ValueError as exc:
        return RedirectResponse(
            url=f"/ui/settings/assembly-lines/{ref.line_id}/references?reference_id={reference_id}&error={quote_plus(str(exc))}",
            status_code=303,
        )
    return RedirectResponse(
        url=f"/ui/settings/assembly-lines/{ref.line_id}/references?reference_id={reference_id}&message=Material+added",
        status_code=303,
    )


@router.post(
    "/settings/assembly-lines/references/{reference_id}/materials-in/{material_id}/qty",
    response_model=None,
)
def update_reference_material_in(
    reference_id: int,
    material_id: int,
    request: Request,
    qty_per_piece: str = Form(...),
    ...
    return RedirectResponse(
        url=f"/ui/settings/assembly-lines/{ref.line_id}/references?reference_id={reference_id}&message=Material+updated",
        status_code=303,
    )


@router.post(
    "/settings/assembly-lines/references/{reference_id}/materials-in/{material_id}/remove",
    response_model=None,
)
def remove_reference_material_in(
    reference_id: int, material_id: int, request: Request, db: Session = Depends(get_db)
):
    enforce_admin(_current_user(request))
    ref = settings_service.get_assembly_line_reference(db, reference_id)
    if ref is None:
    ...
    return RedirectResponse(
        url=f"/ui/settings/assembly-lines/{ref.line_id}/references?reference_id={reference_id}&message=Material+removed",
        status_code=303,
    )


@router.post(
    "/settings/assembly-lines/references/{reference_id}/materials-out/add", response_model=None
)
def add_reference_material_out(
    reference_id: int,
    request: Request,
    part_number: str = Form(...),
    qty_per_piece: str = Form(...),
    ...
    return RedirectResponse(
        url=f"/ui/settings/assembly-lines/{ref.line_id}/references?reference_id={reference_id}&message=Outcome+material+added",
        status_code=303,
    )


@router.post(
    "/settings/assembly-lines/references/{reference_id}/materials-out/{material_id}/qty",
    response_model=None,
)
def update_reference_material_out(
    reference_id: int,
    material_id: int,
    request: Request,
    qty_per_piece: str = Form(...),
    ...
    try:
        ...
        if parsed_qty is None:
            raise ValueError("Qty / piece is required.")
        settings_service.update_reference_material_out_qty(
            db, reference_id, material_id, parsed_qty
        )
    except ValueError as exc:
        return RedirectResponse(
            url=f"/ui/settings/assembly-lines/{ref.line_id}/references?reference_id={reference_id}&error={quote_plus(str(exc))}",
    ...
    return RedirectResponse(
        url=f"/ui/settings/assembly-lines/{ref.line_id}/references?reference_id={reference_id}&message=Outcome+material+updated",
        status_code=303,
    )


@router.post(
    "/settings/assembly-lines/references/{reference_id}/materials-out/{material_id}/remove",
    response_model=None,
)
def remove_reference_material_out(
    reference_id: int, material_id: int, request: Request, db: Session = Depends(get_db)
):
    enforce_admin(_current_user(request))
    ref = settings_service.get_assembly_line_reference(db, reference_id)
    ...
    return RedirectResponse(
        url=f"/ui/settings/assembly-lines/{ref.line_id}/references?reference_id={reference_id}&message=Outcome+material+removed",
        status_code=303,
    )
