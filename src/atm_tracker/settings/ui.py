...
def render_global_settings() -> None:
    init_db()
    page_header("⚙️ Global Settings", "Manage shared reference data used across actions.")
    st.divider()
    kpi_row([])
    st.divider()
        ...
        with main:
            ...
            if isinstance(current_selection, int) and current_selection not in options:
                ...
                if selected_champion:
                    label = (
                        selected_champion.get("name_display")
                        or selected_champion.get("name")
                        or f"ID {current_selection}"
                    )
            ...
            selected = st.selectbox(
                ...
                else options.get(int(value), str(value)),
                key="champion_select",
            )
